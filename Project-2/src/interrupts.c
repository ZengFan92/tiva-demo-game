...
extern volatile bool dataIsReceived;
...
void SysTick_Handler(void)
{
  ...
  Alert_1ms = true;
	//CommentsSystick: Use SysTick timer with interrupts de-bounce push buttons
			...
			//CommentsEEPROM,CommentsPB. modify contents in EEPROM
				//and reboot device.
						if(ps2IsPress)
					{
							DisableInterrupts();
...
//{
//	SSI0_Type *mySSI = (SSI0_Type *)RF_SPI_BASE ;
//	data_r = mySSI->DR & 0xFFFF;
//	mySSI->ICR;
//	
//}
//*****************************************************************************
...
// CommentsWireless: receive interrupt handler
void GPIOD_Handler(void)
{
	GPIOA_Type *gpioPort = (GPIOA_Type *) GPIOD_BASE;
	if((gpioPort->MIS & RF_IRQ_PIN) == RF_IRQ_PIN)
	{
		dataIsReceived = true;
	}
