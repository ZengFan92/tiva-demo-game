...
//extern uint32_t numPacketsTried;
//extern uint32_t numPacketsLost;
...
static __INLINE uint8_t wireless_reg_read(uint8_t reg)
{
	...
	returnData[1] = 0;
	wireless_CSN_low();
	spiTx(wirelessPinConfig.wireless_spi_base,sendData,2,returnData);
	wireless_CSN_high();
	return returnData[1];
}
...
static __INLINE void wireless_reg_write(uint8_t reg, uint8_t data)
{
	...
	uint8_t sendData[2];
	uint8_t returnData[2];
	command |= NRF24L01_CMD_W_REGISTER;
	sendData[0] = command;
	sendData[1] = data;
	wireless_CSN_low();
	spiTx(wirelessPinConfig.wireless_spi_base,sendData,2,returnData);
...
static __INLINE void wireless_set_tx_addr(uint8_t  *tx_addr)
{
	...
	sendData[0] = command;
	sendData[1] = tx_addr[0];
	sendData[2] = tx_addr[1];
	sendData[3] = tx_addr[2];
	sendData[4] = tx_addr[3];
	sendData[5] = tx_addr[4];
	wireless_CSN_low();
	spiTx(wirelessPinConfig.wireless_spi_base,sendData,6,returnData);
	wireless_CSN_high();
}
...
static __INLINE void wireless_tx_data_payload( uint32_t data)
{
	...
	uint8_t secondByte = (data & 0xFF00) >> 8;
	uint8_t thirdByte = (data & 0xFF0000) >> 16;
	uint8_t fourthByte = (data & 0xFF000000) >> 24;
	uint8_t sendData[5];
	uint8_t returnData[5];
	sendData[0] = command;
	sendData[1] = firstByte;
	sendData[2] = secondByte;
	sendData[3] = thirdByte;
	sendData[4] = fourthByte;
...
static __INLINE void wireless_rx_data_payload( uint32_t *data)
{
	...
	spiTx(wirelessPinConfig.wireless_spi_base,sendData,5,returnData);
	wireless_CSN_high();
	firstByte = returnData[1];
	secondByte = returnData[2];
	thirdByte = returnData[3];
	fourthByte = returnData[4];
	*data = 0;
	*data |= firstByte;
	*data |= (secondByte << 8);
	*data |= (thirdByte << 16);
...
)
{
  ...
  if( spiVerifyBaseAddr(wirelessPinConfig.wireless_spi_base))
  {
    ...
    // Enable the Radio in RX mode, disable the interrupt for the transmitter and disable the receiving mask
    wireless_reg_write(NRF24L01_CONFIG_R, NRF24L01_CONFIG_PWR_UP | NRF24L01_CONFIG_EN_CRC | NRF24L01_CONFIG_PRIM_RX_PRX | (1 << 5) | (1 << 4) );
